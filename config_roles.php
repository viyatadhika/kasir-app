...
if (!function_exists('requireAccess')) {
    ...
    function requireAccess($page = '')
    {
        ...
        if (!canAccessPage($role, $currentPage)) {
            ...
            echo '<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Akses Ditolak - SEJAHUB</title><script src="https://cdn.tailwindcss.com"></script></head><body class="min-h-screen flex items-center justify-center bg-gray-50"><div class="text-center p-8 max-w-sm"><p class="text-6xl font-black text-gray-200 mb-4">403</p><h1 class="text-lg font-bold mb-2">Akses Ditolak</h1><p class="text-sm text-gray-500 mb-6">Role <strong>' . $safeRole . '</strong> tidak diizinkan mengakses halaman <strong>' . $safePage . '</strong>.</p><a href="dashboard.php" class="inline-block bg-black text-white text-xs font-bold uppercase tracking-widest px-6 py-3 hover:bg-gray-800">Kembali ke Dashboard</a></div></body></html>';
            exit;
        }
    }
}
